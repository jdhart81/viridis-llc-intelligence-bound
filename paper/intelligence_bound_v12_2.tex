...
With bounded memory capacity $C$ bits, sustained learning at rate $\dot{I} > 0$ requires bit erasure at rate $\geq \dot{I}$, hence power dissipation $P \geq \dot{I} \cdot k_B T \ln 2$.
\end{lemma}
\begin{proof}
Suppose $\dot{I} > 0$ is sustained. Then cumulative information $\int_0^t \dot{I}(\tau) d\tau \to \infty$ as $t \to \infty$. But $H(Y_t) \leq C$ for all $t$ by assumption. By the pigeonhole principle, to incorporate new information, old information must be overwritten. Forgetting is bit erasure: mapping many possible past states to a single ``forgotten'' state. By Axiom 1 (Landauer), each erased bit dissipates $\geq k_B T \ln 2$. In steady state, the forgetting rate must equal at least the learning rate: $\dot{I}_{\text{forget}} \geq \dot{I}$. Therefore $P \geq \dot{I} \cdot k_B T \ln 2$. $\square$
\end{proof}
