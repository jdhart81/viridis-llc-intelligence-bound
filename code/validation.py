...
def validate_real_systems():
    ...
    for name, analysis in systems.items():
        print(f"{name:<18} {analysis.data_bound:>14.2e} "
    ...
    print(f"Headroom range: 10^{np.log10(min(s.headroom for s in systems.values())):.0f} "
...
def validate_biodiversity_impact():
    ...
    print(f"Cascade model: D_after = D₀ × (1-f)^(1+2f)")
    
    print(f"\n{'Loss':>8} {'D_after':>10} {'Ceiling [bits/s]':>18} {'Reduction':>12}")
    print("-" * 52)
    
    for r in results:
        print(f"{r.loss_fraction:>8.0%} {r.D_after:>10.3f} "
              f"{r.ceiling_after:>18.2e} {r.ceiling_reduction_pct:>11.1f}%")
    
    # Verify nonlinearity (50% loss should cause >50% ceiling reduction if cascade works)
    loss_50 = next(r for r in results if r.loss_fraction == 0.5)
    nonlinear = loss_50.ceiling_reduction_pct > 50 * (loss_50.D_before / 0.5)
...
def validate_strategy_comparison():
    ...
    print(f"Exploit: λ = 0.02/year, Regenerate: r = 0.005/year")
    
    print(f"\n{'Strategy':<12} {'Final D':>10} {'Total Intelligence':>22} {'Outcome':>12}")
    print("-" * 60)
    
    for name, r in strategies.items():
        print(f"{name:<12} {r.final_D:>10.3f} {r.total_intelligence:>22.2e} {r.outcome:>12}")
...
def validate_crossover():
    ...
    print(f"Approximate result: t* ≈ 1/λ")
...
def save_results(output_dir: str = "output"):
    ...
    with open(os.path.join(output_dir, "table_i_temperature_scaling.csv"), "w") as f:
        ...
        for i in range(len(table["temperature_K"])):
            f.write(f"{table['temperature_K'][i]},"
    ...
    with open(os.path.join(output_dir, "table_ii_real_systems.csv"), "w") as f:
        systems = real_systems_table()
        f.write("system,power_W,temperature_K,bandwidth_bits_s,D,"
                "data_bound,landauer_bound,headroom,limiting\n")
        for name, s in systems.items():
            f.write(f"{name},{s.power},{s.temperature},{s.bandwidth},{s.D},"
    ...
    with open(os.path.join(output_dir, "table_iv_biodiversity.csv"), "w") as f:
        ...
        for r in results:
            f.write(f"{r.loss_fraction},{r.D_after:.6f},"
